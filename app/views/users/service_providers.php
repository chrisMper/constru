<?php
    include ("connection.php");
?>

<html>
    <head>
        <title>Admins</title>
        <link rel="stylesheet" type="text/css" href="../../../public/css/service_providers.css" >
    </head>

	<body> 
       
        <div class ="navbar">
          <div class="navlogo">
            <img src="../../../public/img/product_img/logo.png" >
          </div>

          <div class="navright">
        
           <a href="profile.html" target="supplierDashboard"> <img src="../../../public/img/product_img/profileicon.png" ></a>
           <a href="cart.html" target="supplierDashboard"> <img src="../../../public/img/product_img/cart.png"></a>
    
          </div>
          <div class="navcenter">

            <ul>
              <li><h2><a href="landingpage.html">Home</a></h2></li>
              <li><h2><a href="aboutus.html">About Us</a></h2></li>
              <li>
                <button class="dropbtn">
                  <h2>Our Services</h2>
                </button>
                  <div class="servicemenu">
                    <a href="engineer.html">Civil Engineering</a></br>
                    <a href="construction.html">Construction</a></br>
                    <a href="hardware.html">Hardware Products & Supplies</a></br>
                    <a href="tender.html">Tendering</a>
                  </div>
              </li> 
            </ul>
          </div>
   
      </div>  


     <div class="table-container">
      <div class="sidebar">
        <ul>
          <li><a href="landingpage.html"><img src="../../../public/img/product_img/home2.png" >Home</a></li>
          <li><a href="landingpage.html"><img src="../../../public/img/product_img/dashboard.png" >Dashboard</a></li>
          <li ><a href="join_requests.php"><img src="../../../public/img/product_img/message.png" >Join requests</a>
          <li><a href="engineer.html"><img src="../../../public/img/product_img/order.png" >Complaints</a></li>
          <li class="active" ><a href="construction.html"><img src="../../../public/img/product_img/listings.png" >Service providers</a></li>
          <li><a href="hardware.html"><img src="../../../public/img/product_img/cart.png" >Admins</a></li>
          <li><a href="tender.html"><img src="../../../public/img/product_img/myorder.jpg" >Profile</a></li>
          <li><a href="myBookings_ongoing.php"><img src="../../../public/img/product_img/booking-icon.jpg" >My statistics</a></li>
          <li><a href="tender.html"><img src="../../../public/img/product_img/statistics.jpg" >Calender</a></li>
        </ul>
      </div>
    </div>
                               
    <div class="main">     
    
        <div class="registerbox">

        <h2>Engineer</h2>
            <table>
                <thead>
                  <th>Name</th>
                  <th>Email</th>
                  <th>Role</th>
                  <th>Address line 1</th>
                  <th>Address line 2</th>
                  <th>City</th>
                  <th>District</th>
                  <th>Country</th>
                  <th>User ID</th>

                </thead>

                <tbody>
                    
                              <tr>
                                
                                <td><center><b>Yasintha Samaranayake </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yasinthags99@gmail.com</td>
                                
                                <td>Engineer</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E201</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Madusha Silva</b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>madushadesilva@gmail.com	</td>
                                
                                <td>Engineer</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E203</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Chris Perera </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yggs46@gmail.com</td>
                                
                                <td>Engineer</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E204</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Yohan Buddhika </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yohanbuddika33@gmail.com</td>
                                
                                <td>Engineer</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E206</td>
                              
                                </tr>
                                             
                </tbody>
            </table>

            <h2>Construction Company</h2>
            <table>
                <thead>
                <th>Name</th>
                  <th>Email</th>
                  <th>Role</th>
                  <th>Address line 1</th>
                  <th>Address line 2</th>
                  <th>City</th>
                  <th>District</th>
                  <th>Country</th>
                  <th>User ID</th>

                </thead>

                <tbody>
                    
                              <tr>
                                
                                <td><center><b>Yasintha Samaranayake </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yasinthags99@gmail.com</td>
                                
                                <td>Construction Company</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E201</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Madusha Silva</b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>madushadesilva@gmail.com	</td>
                                
                                <td>Construction Company</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E203</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Chris Perera </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yggs46@gmail.com</td>
                                
                                <td>Construction Company</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E204</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Yohan Buddhika </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yohanbuddika33@gmail.com</td>
                                
                                <td>Construction Company</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E206</td>
                              
                                </tr>

                                
              
                </tbody>
            </table>

            <h2>Supplier</h2>
            <table>
                <thead>
                <th>Name</th>
                  <th>Email</th>
                  <th>Role</th>
                  <th>Address line 1</th>
                  <th>Address line 2</th>
                  <th>City</th>
                  <th>District</th>
                  <th>Country</th>
                  <th>User ID</th>

                </thead>

                <tbody>
                    
                              <tr>
                                
                                <td><center><b>Yasintha Samaranayake </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yasinthags99@gmail.com</td>
                                
                                <td>Supplier</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E201</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Madusha Silva</b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>madushadesilva@gmail.com	</td>
                                
                                <td>Supplier</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E203</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Chris Perera </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yggs46@gmail.com</td>
                                
                                <td>Supplier</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E204</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Yohan Buddhika </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yohanbuddika33@gmail.com</td>
                                
                                <td>Supplier</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E206</td>
                              
                                </tr>
              
                </tbody>
            </table>

            </table>

            <h2>Customer</h2>
            <table>
                <thead>
                <th>Name</th>
                  <th>Email</th>
                  <th>Role</th>
                  <th>Address line 1</th>
                  <th>Address line 2</th>
                  <th>City</th>
                  <th>District</th>
                  <th>Country</th>
                  <th>User ID</th>

                </thead>

                <tbody>
                    
                              <tr>
                                
                                <td><center><b>Yasintha Samaranayake </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yasinthags99@gmail.com</td>
                                
                                <td>Customer</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E201</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Madusha Silva</b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>madushadesilva@gmail.com	</td>
                                
                                <td>Customer</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E203</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Chris Perera </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yggs46@gmail.com</td>
                                
                                <td>Customer</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E204</td>
                              
                                </tr>

                                <tr>
                                
                                <td><center><b>Yohan Buddhika </b><br><img src="../../../public/img/product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>yohanbuddika33@gmail.com</td>
                                
                                <td>Customer</td>
                                <td> 9/4</td>
                                <td>uyanwatta</td>
                                <td>Miriswatte</td>
                                <td>Gampaha</td>
                                <td>sri lanka</td>
                                <td>E206</td>
                              
                                </tr>
              
                </tbody>
            </table>
        </div>
    </div>
  </body>
</html> 



<!--
                    //create a query
                        $sql = "SELECT * FROM supplier";
                    //run the query
                        $result = mysqli_query($con,$sql);
                    //check results
                        $resultCheck = mysqli_num_rows($result);
                    //condition
                        if($resultCheck > 0){
                            //print results
                            while($row = mysqli_fetch_assoc($result)){
                                
                                $fname = $row['fname'];
                                $lname = $row['lname'];
                                $email = $row['email'];
<td>'.$role.'</td>
                                <td>'.$status.'</td>
                                <td>'.$password.'</td>
                                <td>'.$adLine1.'</td>
                            <td>'.$adLine2.'</td>
                            <td>'.$city.'</td>
                            <td>'.$postalcode.'</td>
                             $district = $row['district'];
                                $country  = $row['country'];
                              //  $userId = $row['userId'];
                               echo '<tr><style="border:5px solid black">
                                
                                <td><center><b>'.$fname.' <?php echo" " ?> '.$lname.'</b><br><img src="product_img/product_placeholder.png" height=100px></center></td>
                                
                                <td>'.$email.'</td>
                                
                                <td>'.$district.'</td>
                                <td>'.$country.'</td>
                                <td>'.$userId.'</td>
                                <td><center><button>View </button></center></td>
                               </style>
                                </tr>';
                            }
                        }
